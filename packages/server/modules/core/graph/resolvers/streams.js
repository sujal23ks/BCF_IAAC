...
module.exports = {
  Query: {
    ...
    async streamsByIds(parent, args, context, info) {
      console.log("streamIds", args.streamIds);
      let data = await getStreams({
        offset: args.offset,
      });

      let streams = data.streams.filter((stream) =>
        args.streamIds.includes(stream.id)
      );
    ...
    async streamRevokePermission(parent, args, context, info) {
      ...
      if (context.userId === args.permissionParams.userId)
        throw new ApolloError(
          "You cannot revoke your own access rights to a stream."
        );
