...
module.exports = {
  async createStream({
    ...
    let stream = {
      ...
      materialThinkness: materialThinkness,
      paid: paid,
      tools: tools,
  ...
  async revokePermissionsStream({ streamId, userId }) {
    ...
    let aclEntry = await Acl()
      .where({ resourceId: streamId, userId: userId })
      .select("*")
      .first();

    if (aclEntry.role === "stream:owner") {
      let ownersCount = Acl().count({
        resourceId: streamId,
        role: "stream:owner",
      });
      if (ownersCount === 1)
  ...
  async getUserStreams({ userId, limit, cursor, publicOnly, searchQuery }) {
    ...
    return {
      streams: rows,
      cursor:
        rows.length > 0 ? rows[rows.length - 1].updatedAt.toISOString() : null,
    };
  },
